...
> **Grup Keuangan** di menu kini merupakan satu view `keuangan.mitra` — untuk **Admin Mitra**: 2 sub-tab — **"Ringkasan"** (kartu saldo utama tunggal, filter chip Uang Masuk/Uang Keluar, tabel mutasi gabungan dengan filter tanggal, tombol Top Up Saldo & Tarik Saldo) dan **"Pembayaran Pelanggan"** (donut legend lunas/belum dibayar, 2 sub-tab: "Belum Bayar" dengan DataTable invoice + tombol Bayar per baris & Bayar Semua & Simulasi Bayar VA, "Riwayat Bayar" dengan filter bulan + tombol detail). Admin Mitra mengajukan penarikan saldo via tombol "Tarik Saldo"; `saldo_utama` berkurang ketika **Tim FAT** menyetujui settlement request. Untuk **Tim FAT**: multi-tab dengan landing "Ringkasan Lintas Mitra" + filter mitra di 3 tab existing (Historis Pembayaran, Verifikasi Deposit, Monitoring Settlement). Modul `deposit.dashboard` dan `settlement.dashboard` masih ada sebagai entry point terpisah namun tidak lagi muncul di navigasi.
...
| `ICONS` | 33 ikon SVG inline | `app-ui.js` |
...
| 2026-08-04 | Bukti pembayaran invoice disederhanakan: hapus format Virtual Account, Alfamart, dan Indomaret; tampilkan rekening bank serta nomor telepon mitra. | `app-modules.js`, `PRD.md` |
...
| 2026-08-04 | **Filter Uang Masuk/Keluar + Simulasi Bayar VA + Invoice Customer + Sub-tab Pembayaran**: (1) **Filter Chip diubah**: filter Ringkasan dari "Semua/Deposit/Settlement/Pembayaran" menjadi "Semua/Uang Masuk/Uang Keluar" dengan tipe mutasi baru `uang_masuk` (deposit, pelanggan_bayar_va, pelanggan_bayar_cash) dan `uang_keluar` (penarikan_settlement). Icon `arrowUpCircle` & `arrowDownCircle` ditambahkan ke `ICONS`. `buildMutations()` diupdate: `depositHistory` → `uang_masuk` (Deposit Saldo) atau `uang_keluar` (Penarikan Saldo Settlement); `payments` → `uang_masuk` (Pelanggan Bayar VA) atau `uang_keluar` (Pelanggan Bayar Cash); `settlements` → `uang_keluar` (Penarikan Saldo Settlement). (2) **Invoice Customer**: fungsi `buildCustomerInvoiceHTML()` — layout "Bukti Pembayaran" untuk customer (bukan invoice mitra). `openCustomerInvoiceModal()` — modal dual-tab: "Invoice Mitra" (`buildPaymentInvoiceHTML`) + "Invoice Customer" (`buildCustomerInvoiceHTML`). Ditampilkan setelah konfirmasi pembayaran (single & bulk) dan simulasi bayar VA. (3) **Simulasi Bayar VA**: `openVASimulationModal()` — modal dengan tabel unpaid invoice + tombol "Bayar VA" per baris. Proses: buat payment, potong saldo, FIFO settle invoice, buat depositHistory, tampilkan invoice customer. (4) **Sub-tab Pembayaran Pelanggan**: "Belum Bayar" (DataTable + tombol Bayar per baris + "Bayar Semua" + "Simulasi Bayar VA") dan "Riwayat Bayar" (filter bulan, tombol detail → invoice modal). (5) **Bulk Payment**: setiap customer bayar cash/bulk membuat `depositHistory` individual (type: `pelanggan_bayar_cash`). Setelah bulk selesai, invoice customer pertama ditampilkan. (6) **Bugfix**: `openVASimulationModal` event listener `.act-sim-va` — perbaikan query selector dari `f` (footer) ke `b` (body) karena tombol berada di modal body. | `app-modules.js`, `app-ui.js`, `PRD.md` |
| 2026-08-19 | **Pisah Saldo: Settlement vs Deposit + WO Transfer Settlement FAT + Revisi Struktur Tab Mitra**: (1) **Split Saldo**: `saldo_utama` dihapus — diganti `saldo_settlement` (auto-credit dari VA payment setelah potongan KSO/PG/Admin; tidak bisa dipindahkan ke deposit; pencairan otomatis oleh FAT sesuai jadwal periodik — mingguan tiap Senin, bulanan tiap tanggal 1; Mitra tidak bisa manual withdraw). `saldo_deposit` (untuk pembayaran cash pelanggan; floor min Rp 1.000.000; topup min Rp 1.000.000). Helper baru: `calcDeductions(amount, partner_id)` → `{kso, pg, admin, netAmount}`, `woPeriodLabel(order)` → label periode (1-7). (2) **DB.workOrders**: Entity baru `{id, partner_id, type:'transfer_settlement', period, amount, due_date, status:'Menunggu Proses'|'Selesai', proof, processed_by, processed_at, completed_at}` — 5 dummy records untuk prototipe. (3) **Admin Mitra Tab Revisi**: 2 top-level tabs — **"Settlement"** (kartu saldo_settlement read-only + filter + tabel mutasi settlement; tanpa tombol withdraw; teks "Pencairan otomatis oleh Tim FAT sesuai jadwal") dan **"Deposit"** (kartu saldo_deposit + tombol Top Up + sub-tabs "Mutasi Saldo" & "Pembayaran Pelanggan"). Tab "Ringkasan" lama dihapus. Tombol "Tarik Saldo" dihapus. `openMitraWithdrawModal()` dihapus. `buildMutations(scope)` filter otomatis berdasarkan balance type. Refresh handler hanya handle `settlement`/`deposit`. (4) **FAT Tab Revisi**: 4 top-level tabs: Ringkasan Lintas Mitra, Historis Pembayaran, Verifikasi Deposit, **WO Transfer Settlement**. Tab "Monitoring Settlement" dihapus beserta semua fungsi terkait (`renderSettlementMode`, `renderSettlementQueue`, `renderSettlementMitraOverview`, `openSettlementVerifyModal`). (5) **WO Transfer Settlement FAT**: `renderWOTransferMode()` → kartu stats (Menunggu Proses/Selesai, total nominal); `renderWOPendingQueue()` → DataTable interaktif dengan tombol Proses; `openWOProcessModal()` → form upload bukti transfer + submit otomatis; `renderWOHistory()` → filter per mitra + tabel selesai. Semua fungsi WO dipindahkan ke dalam closure `renderFATKuangan()` agar memiliki akses ke `fatMitraFilter`, `allPartners`, `activeTab`, `renderRoot()`. (6) **STATUS_VARIANT**: Ditambah `Menunggu Proses: 'warning'` dan `Selesai: 'success'` di `app-ui.js`. (7) **CSS Responsif**: Ditambah `@media(max-width:960px){.balance-cards{grid-template-columns:1fr !important;}}` di `erp-mitra-prototype.html`. | `app-data.js`, `app-modules.js`, `app-ui.js`, `erp-mitra-prototype.html` |
